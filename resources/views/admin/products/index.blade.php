            ...
            <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg flex flex-col gap-y-5">
                ...
                <div class="flex flex-row justify-between items-center mb-5">
                    ...
                    <a href="{{ route('admin.products.create') }}"
                        class="rounded-full py-3 px-5 bg-indigo-950 text-white w-fit">Add New
                        Product</a>
                </div>
                    ...
                    <div class="item-product flex flex-row justify-between items-center">
                        ...
                        <div class="flex flex-row gap-x-3">
                            <a href="{{ route('admin.products.edit', $product) }}"
                                class="rounded-full py-3 px-5 bg-indigo-500 text-white font-bold">Edit</a>
                            <form action="{{ route('admin.products.destroy', $product) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="rounded-full py-3 px-5 bg-red-500 text-white">Delete</button>
                            </form>
                        </div>
                    </div>
                @empty
